...
class DensityLookup(BaseAction):
    """Class to handle density lookup action."""

    def execute(self):
        """Execute density lookup action.
    ...
    def __validate_params(self):
        ...
        if "qtype" in self._param:
            ret_val, value = self._connector.validator.validate_dropdown(
                ...
                "qtype",
                consts.DENSITY_LOOKUP_QTYPE_OPTIONS,
            )
        ...
        if "scope" in self._param:
            ret_val, value = self._connector.validator.validate_dropdown(
                self._action_result,
                self._param.get("scope"),
                "scope",
                consts.DENSITY_LOOKUP_SCOPE_OPTIONS,
            )

            if not ret_val:
                return ret_val

            self._param["scope"] = value
