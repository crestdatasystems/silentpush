...
class GetFutureAttackFeed(BaseAction):
    ...
    def execute(self):
        ...
        self._connector.save_progress(
            consts.EXECUTION_START_MESSAGE.format(
    ...
    def __validate_params(self):
        ...
        if "page_no" in self._param:
            ret_val, value = self._connector.validator.validate_integer(
                self._action_result, self._param.get("page_no"), "page_no"
            )
        ...
        if "page_size" in self._param:
            ret_val, value = self._connector.validator.validate_integer(
                self._action_result, self._param.get("page_size"), "page_size"
            )
    ...
    def __handle_response(self, ret_val, response):
        ...
        return self._action_result.set_status(
            phantom.APP_SUCCESS, consts.ACTION_FUTURE_ATTACK_FEED_SUCCESS_RESPONSE
        )
