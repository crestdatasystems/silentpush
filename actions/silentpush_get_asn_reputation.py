...
class GetAsnReputation(BaseAction):
    ...
    def execute(self):
        ...
        self._connector.save_progress(
            consts.EXECUTION_START_MESSAGE.format("get_asn_reputation")
        )
    ...
    def __get_query_params(self):
        ...
        query_params = {"explain": "explain", "limit": "limit"}
    ...
    def __handle_response(self, ret_val, response):
        ...
        summary = {
            "total_asn_reputations": len(
                response.get("response", {}).get("asn_reputation_history", [])
            )
        }
        self._action_result.update_summary(summary)

        return self._action_result.set_status(
            phantom.APP_SUCCESS, consts.ACTION_ASN_REPUTATION_SUCCESS_RESPONSE
        )
